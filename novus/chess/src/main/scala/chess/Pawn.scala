...
case class Pawn(var p: Position, c: Color) extends Piece("Pawn", c, p) {
  ...
  private def killPosition2(board: Board, p: Position) = (onePositionForward(p), board.isPositionOccupied(onePositionForward(p).get.minusFile, Color.other(color))) match {
    case (Some(x), true) => x.minusFile
    case _ => None
  }

  private def killPosition1(board: Board, p: Position) = (onePositionForward(p), board.isPositionOccupied(onePositionForward(p).get.plusFile, Color.other(color))) match {
